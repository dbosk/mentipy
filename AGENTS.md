...
- **The `Question` base class owns the shared presentation flags.** All question dataclasses subclass `Question` (`questions.nw`), which declares the keyword-only `next` and tri-state `results` fields, appends them in `to_dict()` via each subclass's `_own_dict()`, and provides the default `canonical_dict()` subtraction. A new question type implements `_own_dict()` (kind-specific persistence fields only) and overrides `canonical_dict()` only when identity is narrower than persistence. Keep the shared fields keyword-only — positional defaults on the base would break subclass positional construction.
