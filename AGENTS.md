...
- **A new shared per-question flag** (like `next`, `results`, `lock`) touches the `Question` base class in `questions.nw` (field, `to_dict()`, `canonical_dict()`, `_shared_fields()`, the explicit `to_dict()` expectations in its tests), `PinImageQuestion.from_image_bytes`, all seven wrappers in `latex.nw` plus the `HELPER_FACTORIES` test, all seven `render` subcommands in `cli.nw` plus a `QUESTION_<X>_OPTION`, the `_latex_reference()` signatures and "Per-question keyword arguments" text with its test, and the manual.
- **A new `mentipy serve` flag** touches `serve` and `_serve_publicly` in `cli.nw`, and `serve()`, `create_server()` and `_build_handler()` in `server.nw`; the CLI tests spy on `serve_http` with `**kwargs`, so add a thin test per branch.
...
- **The `Question` base class owns the shared fields and serialisation entry points.** All question dataclasses subclass `Question` (`questions.nw`), which declares the prompt `text` plus the keyword-only `next` and tri-state `results` flags, writes `kind`/`text`/flags around each subclass's `_own_dict()` in `to_dict()`, provides the default `canonical_dict()` subtraction, and extracts the shared fields in `_shared_fields()` during rebuild. The two pin types share a further `PinQuestion` base carrying the keyword-only `max_pins`/`allow_comment` contract (validated in its `__post_init__`; subclasses must chain `super().__post_init__()`); dispatch sites use `isinstance(q, PinQuestion)` — the old `PIN_QUESTIONS` tuple is gone. A new question type subclasses one of these bases, implements `_own_dict()` and `_from_dict()`, joins the `QUESTION_KINDS` registry, and overrides `canonical_dict()` only when identity is narrower than persistence. Keep shared default-valued fields keyword-only — positional defaults on a base would break subclass positional construction.
- **Per-question results visibility is a tri-state.** `results=None` defers to `mentipy serve --results`; `True`/`False` override it per question at both server decision points (post-submit redirect and the GET results gate, resolved by `_results_public()` in `server.nw`). Admin/local access is checked independently and must never be blocked by the override.
- **Lecture progress is session state, never store state.** `mentipy serve` paces respondents by default (`--no-lock` disables it): a question is open to respondents only after an admin GET of its `/results` page this server run, or when it has `lock=False` and the question before it in the deck is open (transitively; a `lock=False` head is open). "Before it in the deck" is the store's full registration order as returned by `Store.all_questions()` (orphans included), deliberately *not* the navigation row's live suffix: re-registering one existing question (a re-run PythonTeX block, a `mentipy render` in a terminal) moves it to the end of the store, and pacing on the full order guarantees that can never open a question the lecturer has not reached. The `LectureProgress` tracker lives in the `_build_handler()` closure in `server.nw`, holds no store reference and takes the deck rows as data, so a GET never mutates `mentipy.json` and a restart re-locks everything (the lecturer's refreshing results tab reopens the current question). Set semantics: skipped questions stay locked. Only the routes that reveal a question are gated (form, image, results, via `_resolve_for_respondent()`); POST and `/thanks` are deliberately not. `/active` redirects to the most recently *newly* opened question (first-open-wins, so a stale refreshing tab cannot move it) and shows a self-refreshing waiting page before any opening or when that question has left the store; the tracker records openings even under `--no-lock` so `/active` keeps working. The locked page links to `/active` only when the lecturer has moved past the locked question. The admin index labels each question active/open/locked with the same rule. Because `/active` follows admin requests, admin is now a write capability: a tunnel that makes students look local lets them steer the room.
